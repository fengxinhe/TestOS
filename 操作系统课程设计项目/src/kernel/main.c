...
void help()
{
	...
	printf("4. clock         : Show the system time\n");
...
void clock(fd_stdin){
	...
	if(initial == 0)
	{
		printf("Please initialize the time...\n");
		printf("Please enter the year : ");
		while(1)
		{
			char year[80] = {0};
			read(fd_stdin, year, 80);
			atoi(year, &t.year);
			//printf("test....year: %d\n",t.year);
			...
			{
				printf("Input wrong!Please enter the right number!\n");
				printf("Please enter the year : ");
			}
		}
		...
		{
			char minute[80] = {0};
			read(fd_stdin, minute, 80);
			atoi(minute, &t.minute);
		...
		{
			char second[80] = {0};
			read(fd_stdin, second, 80);
			atoi(second, &t.second);
			//printf("test....second: %d\n",t.second);
	...
	{	
		...
		printf("Will back to the main menu in %d seconds...", back);
		update();
		milli_delay(17000);
		back--;
		if(back == -1)
